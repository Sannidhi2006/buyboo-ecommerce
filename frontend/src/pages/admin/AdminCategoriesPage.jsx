  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingCategory) {
        ...
        setSuccessMsg(`Category "${form.name}" updated successfully.`);
      } else {
        await api.post('/categories', formData);
        setSuccessMsg(`Category "${form.name}" created successfully.`);
      }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1 ml-10">
            Manage storefront categories, navigation hierarchy, and image assets.
          </p>
        </div>
              ...
              <tbody className="divide-y divide-slate-700/50 text-sm">
                {filteredCategories.map((cat) => (
                  <tr key={cat.id} className="hover:bg-slate-700/20 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <span className="font-mono text-xs px-2 py-1 bg-slate-900/80 rounded-md border border-slate-700/60 text-indigo-300">
                        {cat.slug}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-slate-800 border border-rose-500/40 rounded-2xl max-w-sm w-full p-6 shadow-2xl text-center">
            ...
            <p className="text-sm text-slate-400 mb-6">
              Are you sure you want to delete this category? If any products reference it, deletion
